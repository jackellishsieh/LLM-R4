...
from src.python_stdout_engine import (
    run_python_stdout_code,
    compare_both_string_and_number_format,
    number_it,
)
...
def prepare_datasets_and_data_loaders(args, tokenizer):
    with accelerator.main_process_first():
        ...
        raw_dataset = DatasetDict(
            {
                "train": (
                    Dataset.from_list(json.load(open(args["train_file"], "r")))
                    if not args["train_file"].rstrip("/").endswith("_cache")
                    else load_from_disk(args["train_file"])
                ),
                "test": (
                    Dataset.from_list(json.load(open(args["test_file"], "r")))
    ...
    return (
        ...
        (tokenized_dataset["test"], test_dataloader),
        cot_info,
    )
...
def logging_values(
    ids, vpreds, rets, advs, old_vpreds, rews, score_rews, masks, tokenizer
):

    get_str_digits = lambda x: str(round(x.item(), 3))

    for tmp_i in range(ids.size(0)):
        mk_ids = torch.masked_select(
            ids[tmp_i], masks[tmp_i].bool()
        )  # (valid_resp_len,)
        mk_vpreds = torch.masked_select(
            vpreds[tmp_i], masks[tmp_i].bool()
        )  # (valid_resp_len,)
        mk_rets = torch.masked_select(
            rets[tmp_i], masks[tmp_i].bool()
        )  # (valid_resp_len,)
        mk_advs = torch.masked_select(
            advs[tmp_i], masks[tmp_i].bool()
        )  # (valid_resp_len,)
        mk_old_vpreds = torch.masked_select(
            old_vpreds[tmp_i], masks[tmp_i].bool()
        )  # (valid_resp_len,)
        mk_rews = torch.masked_select(
            rews[tmp_i], masks[tmp_i].bool()
        )  # (valid_resp_len,)
        mk_score_rews = torch.masked_select(
            score_rews[tmp_i], masks[tmp_i].bool()
        )  # (valid_resp_len,)
...
def rollout(
    ...
    for completed_ids, a_value in zip(completed_tensors, answer_values):
        ...
        completed_text: str = tokenizer.decode(
            completed_ids.cpu().numpy().tolist(), skip_special_tokens=True
        )

        # attempt to extract the produced answer
        try:
            with timeout(TIMEOUT):
                extracted_ans = post_process_completed_question_answer_fn_mapper[
                    (args["engine"], src_name)
...
def train_one_epoch(
    ...
    for idx, batch in tqdm(
        ...
        if (
            args["logging_seq_str_step_freq"]
            and global_step % args["logging_seq_str_step_freq"] == 0
        ):
        ...
        for _ in range(ppo_epochs):
            ...
            for mini_idx in range(0, len(perms), mini_batch_size_per_gpu):
                ...
                # logging values
                if (
                    args["logging_values_step_freq"] is not None
                    and global_step % args["logging_values_step_freq"] == 0
                ):
        ...
        if evaluating_step_freq is not None and global_step % evaluating_step_freq == 0:
            evaluate_result_dict = {
                ...
                for k, v in evaluate_generation(
                    args, model, test_dataset, test_dataloader, tokenizer, cot_info
                ).items()
            }
        ...
        if eval_log_dict or train_log_dict:
            ...
            accelerator.print(
                f"{prefix}[Epoch={epoch}/{max_epoch}, Step={global_step}] {log_dict}"
            )
...
def main(args):
    ...
    MODEL_CLASS = (
        ...
        if not args["separate_vf"]
        else AutoModelForCausalLMWithValueModel
    )
